...
function consolidateBlocks(hex,side,index){
	...
	// ── Particle effects on block clear ──
	if (typeof Particles !== 'undefined') {
		var centerCoords = findCenterOfBlocks(deletedBlocks);
		Particles.blockExplosion(centerCoords.x, centerCoords.y, deletedBlocks[0].color, deleting.length * 3);
		if (hex.comboMultiplier > 1) {
			Particles.comboBurst(centerCoords.x, centerCoords.y, deletedBlocks[0].color, hex.comboMultiplier);
		}
	}

	// ── Stats tracking ──
	if (typeof GameStats !== 'undefined') {
		GameStats.onBlocksCleared(deleting.length);
		if (hex.comboMultiplier > 1) {
			GameStats.onCombo(hex.comboMultiplier);
		}
	}

	// ── Earn power-ups through combos ──
	if (typeof PowerUps !== 'undefined') {
		// 3x combo → earn a random power-up
		if (hex.comboMultiplier === 3) {
			PowerUps.rewardRandom();
		}
		// 5x combo → earn hammer
		if (hex.comboMultiplier === 5) {
			PowerUps.addCharge('hammer');
		}
		// 7x+ combo → earn freeze
		if (hex.comboMultiplier === 7) {
			PowerUps.addCharge('freeze');
		}
		// 10x+ combo → earn color bomb
		if (hex.comboMultiplier >= 10 && hex.comboMultiplier % 5 === 0) {
			PowerUps.addCharge('colorBomb');
		}
		// Big clears (6+ blocks at once) → earn a random power-up
		if (deleting.length >= 6) {
			PowerUps.rewardRandom();
		}
	}

	// ── Multiplayer: Send penalty to opponents ──
	if (typeof MP !== 'undefined' && MP.isMultiplayer && deleting.length >= 3) {
		MP.sendPenalty(deleting.length, hex.comboMultiplier);
	}
}
