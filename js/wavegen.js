...
function waveGen(hex) {
	...
	this.halfCircleGeneration = function() {
		if (this.dt - this.lastGen > (this.nextGen + 500) / 2) {
			...
			var hcSides = MainHex.sides || 6;
			var hcHalf = Math.floor(hcSides / 2);
			var d = randInt(0, hcSides);
			for (var i = 0; i < hcHalf; i++) {
				addNewBlock((d + i) % hcSides, colorList[i % colorList.length], 1.5 + (this.difficulty / 15) * 3);
			}
	...
	this.crosswiseGeneration = function() {
		if (this.dt - this.lastGen > this.nextGen) {
			...
			var oppOffset = Math.floor(MainHex.sides / 2);
			addNewBlock(i, colors[ri], 0.6 + (this.difficulty / 15) * 3);
			addNewBlock((i + oppOffset) % MainHex.sides, colors[ri], 0.6 + (this.difficulty / 15) * 3);
			this.ct += 1.5;
			this.lastGen = this.dt;
			this.shouldChangePattern();
		}
