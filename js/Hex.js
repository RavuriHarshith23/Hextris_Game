function Hex(sideLength) {
	this.playThrough = 0;
	this.fillColor = [30,30,55];
	this.tempColor = [30,30,55];
	this.angularVelocity = 0;
	this.position = 0;
	this.dy = 0;
	this.dt = 1;
	this.sides = (typeof GameLevels !== 'undefined' && GameLevels.getActiveSides()) ? GameLevels.getActiveSides() : ((typeof GameConfig !== 'undefined') ? GameConfig.getHexSides() : 6);
	this.blocks = [];
	...
	this.shake = function(obj) { //lane as in particle lane
		var angle = (180 / this.sides) + obj.lane * (360 / this.sides);
		angle *= Math.PI / 180;
	...
	this.rotate = function(steps) {
		...
		var _rotAngle = 360 / this.sides;
		this.position = this.position % this.sides;
		var _s = steps;
		this.blocks.forEach(function(blocks) {
			blocks.forEach(function(block) {
				block.targetAngle = block.targetAngle - _s * _rotAngle;
			});
		});

		this.targetAngle = this.targetAngle - steps * _rotAngle;
				this.lastRotate = Date.now();

		// Broadcast rotation to multiplayer opponents
		if (typeof MP !== 'undefined' && MP.isMultiplayer) {
			MP.sendEvent('rotate', { steps: steps, angle: this.targetAngle });
		}
	};
